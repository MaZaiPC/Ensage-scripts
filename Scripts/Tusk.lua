			...
			if tick > sleep[1] then
				if not Animations.isAttacking(me) then
					...
					if not shadowplay and victim.alive and victim.visible then
						...
						if E and E:CanBeCasted() and me:CanCast() and distance <= 450 and not me:DoesHaveModifier("modifier_tusk_snowball_movement") then
							table.insert(castQueue,{100,E})
						end
						if PUNCH and PUNCH:CanBeCasted() and me:CanCast() and distance <= 350 and not me:DoesHaveModifier("modifier_tusk_snowball_movement") then
